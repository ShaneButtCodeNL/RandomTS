...
describe("\n01 Constructors", () => {
  ...
  it("04 constructor with seed max min", () => {
    const RNG = new RandomWholeNumber(123456, 321, 111);
    expect(RNG).toBeDefined();
    expect(typeof RNG).toBe("object");
  });
});
...
describe("\n02 Generate Values", () => {
  it("01 Should generate a number from next calls", () => {
    ...
    const RNG2 = new RandomWholeNumber(123456789);
...
describe("\n03 Range functions. Each will call 1000 calls to next", () => {
  it("01 Generateed values should be in range", () => {
    const RNG = new RandomWholeNumber(1234567, 10, 0);
  ...
  it("03 Change min value of range, generate values should be greater then or equal then new min", () => {
    const RNG = new RandomWholeNumber(1234567, 200, 0);
    const newMin = 100;
    RNG.setMin(newMin);
    for (let i = 0; i < 1000; i++) {
      const v = RNG.next();
  ...
  it("05 Change min value to value greater than max should throw error", () => {
    const RNG = new RandomWholeNumber(1234567, 200, 0);
    expect(() => RNG.setMin(9000)).toThrow();
  });
  it("06 Change Max Value to be less than min should throw error", () => {
    const RNG = new RandomWholeNumber(12345467, 1000, 900);
    expect(() => RNG.setMax(900)).toThrow();
  });
});
